...
class SecurityFindingDB(Base):
    ...
    )  # open, resolved, false_positive, ignored
...
class DatabaseManager:
    ...
    def __init__(self, connection_string: Optional[str] = None):
        ...
        if self._should_initialize_database():
            ...
            logger.warning(
                "No database connection string provided - database features disabled"
            )
    ...
    def _should_initialize_database(self) -> bool:
        ...
        return bool(
            ...
            or (
                ...
                and config.azure_sql_database
                and config.azure_sql_username
            )
        )
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            if not sqlalchemy_url:
                logger.warning(
                    "No valid database URL - skipping database initialization"
                )
                return
    ...
    async def save_scan_result(self, scan_result: ScanResult) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to save scan result", scan_id=scan_result.scan_id, error=str(e)
            )
            session.rollback()
            return False
        finally:
        ...
        try:
            for finding in findings:
                try:
                    ...
                    existing = (
                        session.query(SecurityFindingDB)
                        .filter(SecurityFindingDB.id == finding.id)
                        .first()
                    )
                    ...
                    if existing:
                        ...
                        existing.status = (
                            "open"  # Re-open if it was previously resolved
                        )
                    ...
                    logger.error(
                        "Failed to save individual finding",
    ...
    async def save_honeypot_event(self, event: HoneypotEvent) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to save honeypot event", event_id=event.id, error=str(e)
            )
            session.rollback()
            return False
        finally:
    ...
    async def mark_finding_resolved(self, finding_id: str) -> bool:
        ...
        try:
            ...
            finding = (
                session.query(SecurityFindingDB)
                .filter(SecurityFindingDB.id == finding_id)
                .first()
            )
            ...
            if finding:
                ...
                logger.warning(
                    "Finding not found for resolution", finding_id=finding_id
                )
                return False

        except SQLAlchemyError as e:
            logger.error(
                "Failed to mark finding as resolved",
    ...
    async def get_open_findings(
        ...
        try:
            ...
            if subscription_id:
                query = query.filter(
                    SecurityFindingDB.subscription_id == subscription_id
                )
                ...
                result.append(
                    {
                        ...
                        "resource_groups": (
                            scan.resource_groups.split(",")
                            if scan.resource_groups
                            else []
                        ),
                    }
                )
