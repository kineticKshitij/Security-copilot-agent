...
class SecurityFinding(BaseModel):
    ...
    id: str = Field(default_factory=lambda: f"finding-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}")
...
class ComplianceReport(BaseModel):
    ...
    report_id: str = Field(default_factory=lambda: f"report-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}")
