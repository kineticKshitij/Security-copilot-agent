...
class HoneypotMonitor:
    ...
    def __init__(self):
        self.log_path = Path(config.honeypot_log_path) if config.honeypot_log_path else None
        ...
        logger.info("Honeypot monitor initialized",
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.api_endpoint}/events/recent",
                    headers=headers
                ) as response:
    ...
    async def _parse_log_file(self, log_file: Path) -> AsyncGenerator[HoneypotEvent, None]:
        ...
        try:
            ...
            if line.startswith('{'):
                ...
                return HoneypotEvent(
                    id=data.get('id', f"honeypot-{datetime.utcnow().strftime('%Y%m%d-%H%M%S-%f')}"),
            ...
            if match:
                timestamp_str, src_ip, src_port, dst_ip, dst_port, protocol, event_data = match.groups()
    ...
    async def _parse_api_event(self, event_data: Dict[str, Any]) -> Optional[HoneypotEvent]:
        """Parse event data from API response"""
        try:
            return HoneypotEvent(
                id=event_data.get('id', f"api-{datetime.utcnow().strftime('%Y%m%d-%H%M%S-%f')}"),
    ...
    def _calculate_severity(self, threat_indicators: List[str], port: int) -> Severity:
        ...
        dangerous_indicators = ['sql_injection', 'command_injection', 'directory_traversal']
    ...
    def _classify_event_type(self, event_data: str, threat_indicators: List[str]) -> str:
    ...
    async def _process_honeypot_event(self, event: HoneypotEvent) -> None:
        ...
        try:
            logger.info("Processing honeypot event",
                       event_id=event.id,
            ...
            if event.severity in [Severity.CRITICAL, Severity.HIGH]:
                ...
                if finding:
                    ...
                    logger.info("Created security finding from honeypot event",
            ...
            logger.error("Error processing honeypot event", 
            ...
            for finding in findings:
                ...
                if self._is_ip_in_range(event.source_ip, "0.0.0.0/0"):
                    ...
                    if finding['finding_type'] == 'OPEN_TO_INTERNET':
                        ...
                        logger.info("Correlated honeypot event with finding",
                                   event_id=event.id,
                                   finding_id=finding['id'],
    ...
    async def _create_finding_from_event(self, event: HoneypotEvent) -> Optional[SecurityFinding]:
        ...
        try:
            ...
            description = (f"Honeypot detected active attack from {event.source_ip} "
            ...
            if event.threat_indicators:
                description += f" Threat indicators: {', '.join(event.threat_indicators)}."
            ...
            return SecurityFinding(
                ...
                tags=["honeypot", "active-attack", "real-time"] + event.threat_indicators
            )
            
        except Exception as e:
            logger.error("Error creating finding from event", 
                        event_id=event.id, error=str(e))
            return None
    ...
    async def _send_critical_alert(self, event: HoneypotEvent) -> None:
        ...
        try:
            ...
            logger.critical("Critical security alert sent",
                           event_id=event.id,
                           source_ip=event.source_ip,
                           event_type=event.event_type)
    ...
    async def _send_teams_alert(self, message: str) -> None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(config.teams_webhook_url, json=payload) as response:
                    if response.status != 200:
                        logger.error("Failed to send Teams alert", status=response.status)
    ...
    async def get_recent_events(self, hours: int = 24, 
                              severity_filter: Optional[Severity] = None) -> List[HoneypotEvent]:
