...
class GitHubIntegration:
    ...
    def __init__(
        ...
        logger.info(
            "GitHub integration initialized", repo=f"{self.repo_owner}/{self.repo_name}"
        )
    ...
    async def process_scan_results(self, scan_result: ScanResult) -> Dict[str, Any]:
        ...
        results = {"issues_created": 0, "prs_created": 0, "errors": []}

        try:
            logger.info(
                "Processing scan results for GitHub integration",
                ...
                for finding in scan_result.findings:
                    try:
                        ...
                        if issue:
                            ...
                            if (
                                config.create_prs_for_auto_fix
                                and finding.auto_remediable
                                and finding.remediation_script
                            ):
                                try:
                                    pr = await self.create_pr_for_finding(
                                        finding, issue
                                    )
                                    if pr:
                        ...
                        error_msg = (
                            f"Failed to create issue for finding {finding.id}: {str(e)}"
                        )
                        logger.error(
                            "Issue creation failed", finding_id=finding.id, error=str(e)
                        )
                        results["errors"].append(error_msg)
            ...
            if critical_and_high:
                try:
                    summary_issue = await self.create_summary_issue(
                        scan_result, critical_and_high
                    )
                    if summary_issue:
            ...
            logger.info(
                "GitHub integration processing completed",
                scan_id=scan_result.scan_id,
                **results,
            )
    ...
    async def create_issue_for_finding(
        ...
        try:
            ...
            if existing_issue:
                logger.debug(
                    "Issue already exists for finding",
                    finding_id=finding.id,
            ...
            logger.info(
                "Created GitHub issue for security finding",
                finding_id=finding.id,
            ...
            logger.error(
                "Failed to create GitHub issue", finding_id=finding.id, error=str(e)
            )
            return None

    async def create_pr_for_finding(
        self, finding: SecurityFinding, related_issue: Issue
...
"""
            ...
            try:
                ...
                if "path already exists" in str(e).lower():
                    ...
                    file_obj = self.repository.get_contents(
                        script_path, ref=branch_name
                    )
                    ...
                    logger.debug(
                        "Updated existing remediation script file", path=script_path
                    )
                else:
                    raise
...
"""
            ...
            logger.info(
                "Created pull request for security finding",
                finding_id=finding.id,
            ...
            logger.error(
                "Failed to create pull request for finding",
                finding_id=finding.id,
                error=str(e),
            )
            return None
    ...
    async def create_summary_issue(
        self, scan_result: ScanResult, high_severity_findings: List[SecurityFinding]
    ) -> Optional[Issue]:
                ...
                if count > 0:
                    emoji = (
                        ...
                        if severity == Severity.CRITICAL
                            ...
                            else "🟡" if severity == Severity.MEDIUM else "🔵"
                        )
                    )
...
"""
            ...
            critical_findings = [
                f for f in high_severity_findings if f.severity == Severity.CRITICAL
            ]
...
"""
            ...
            high_findings = [
                f for f in high_severity_findings if f.severity == Severity.HIGH
            ]
            if high_findings:
...
"""
            ...
            logger.info(
                "Created summary issue for security scan",
                scan_id=scan_result.scan_id,
                issue_number=issue.number,
    ...
    def _find_existing_issue(self, finding: SecurityFinding) -> Optional[Issue]:
        ...
        try:
            ...
            search_query = (
                f"repo:{self.repo_owner}/{self.repo_name} is:issue {finding.id}"
            )
            ...
            logger.error(
                "Error searching for existing issues",
                finding_id=finding.id,
                error=str(e),
            )
            return None
    ...
    def _get_labels_for_finding(self, finding: SecurityFinding) -> List[str]:
        ...
        labels.append(
            f"resource-{finding.affected_rule.rule_type.value.lower().replace('_', '-')}"
        )
