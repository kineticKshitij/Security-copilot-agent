...
from .models import (
    SecurityRule, SecurityFinding, FindingType, Severity, 
    RuleType, ScanResult
)
...
class SecurityScanner:
    ...
    async def scan_subscription(self, resource_groups: Optional[List[str]] = None) -> ScanResult:
    ...
    async def _get_all_resource_groups(self) -> List[str]:
        ...
        try:
            ...
            resource_client = ResourceManagementClient(
                self.network_client._config.credential,
    ...
    async def _scan_resource_group_nsgs(self, resource_group: str) -> List[SecurityFinding]:
        """Scan all NSGs in a specific resource group"""
        findings = []
        
        try:
            logger.debug("Scanning NSGs in resource group", resource_group=resource_group)
            
            # Get all NSGs in the resource group
            nsgs = self.network_client.network_security_groups.list(resource_group)
            
            for nsg in nsgs:
                logger.debug("Scanning NSG", nsg_name=nsg.name, resource_group=resource_group)
                
                # Scan security rules
                if nsg.security_rules:
                    for rule in nsg.security_rules:
                        security_rule = self._convert_to_security_rule(rule, resource_group, nsg.name)
            ...
            logger.error("Azure error scanning NSGs", 
                        resource_group=resource_group, 
                        error=str(e))
        except Exception as e:
            logger.error("Unexpected error scanning NSGs", 
                        resource_group=resource_group, 
                        error=str(e))
        
        logger.debug("Completed NSG scan", 
                    resource_group=resource_group, 
                    findings_count=len(findings))
        return findings
    
    def _convert_to_security_rule(self, azure_rule: Any, resource_group: str, nsg_name: str) -> SecurityRule:
    ...
    def _is_open_to_internet(self, rule: SecurityRule) -> bool:
        ...
        return (
            ...
            rule.source_address_prefix.lower() in [s.lower() for s in internet_sources]
        )
    ...
    def _is_unrestricted_ssh_rdp(self, rule: SecurityRule) -> bool:
        ...
        risky_ports = ["22", "3389"]
        return (
            self._is_open_to_internet(rule) and
            rule.destination_port_range and
            any(port in rule.destination_port_range for port in risky_ports)
        )
    ...
    def _has_exposed_database_ports(self, rule: SecurityRule) -> bool:
        ...
        return (
            self._is_open_to_internet(rule) and
            rule.destination_port_range and
            any(port in rule.destination_port_range for port in db_ports)
        )
    ...
    def _has_exposed_admin_ports(self, rule: SecurityRule) -> bool:
        ...
        return (
            self._is_open_to_internet(rule) and
            rule.destination_port_range and
            any(str(port) in rule.destination_port_range for port in self.admin_ports.keys())
        )
    ...
    def _uses_weak_protocols(self, rule: SecurityRule) -> bool:
        ...
        return (
            rule.protocol and
            rule.protocol.lower() in weak_protocols
        )
    ...
    def _create_ssh_rdp_finding(self, rule: SecurityRule) -> SecurityFinding:
        ...
        port_info = "SSH (22)" if "22" in str(rule.destination_port_range) else "RDP (3389)"
